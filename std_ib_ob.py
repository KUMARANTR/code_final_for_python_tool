...
def connect_to_database_inbound():
    ...
    try:
        ...
        print(f"Inbound Data ceatenotebook Query: {select_query}")  # Debugging: Print query
...
def inbound_volumes(inbound_data, start_date=None, end_date=None, dc_name=None):
    try:
        ...
        metrics = {
            ...
            f"Total {po_label}": total_orders_formatted,  # Dynamically set label
...
def inbound_order_profile(inbound_data, start_date=None, end_date=None, dc_name=None):
    try:
        ...
        metrics = {
            ...
            f"IB Lines Per {po_label}": ib_lines_per_po_formatted,  # Dynamically set label
            f"IB Units Per {po_label}": ib_units_per_po_formatted
        }
        return metrics, po_label  # Return metrics with the dynamic label
    except KeyError as e:
        messagebox.showerror("Error", f"Missing column in inbound data: {str(e)}")
...
def display_inbound_summary(inbound_metrics, inbound_order_profile_metrics, inbound_data, start_date, end_date, dc_name, po_label):
...
import pandas as pd  # Uncomment if not already imported
...
def export_inbound_outbound_data(
    ...
    try:
        ...
        outbound_data_to_export = {
            ...
            "Total OB Loads": [outbound_metrics.get("Total OB Loads", "N/A")],
